...
archivos_a_cargar <- c("matint_179_mue_dico.Rdata", "matint_179_mue.Rdata")
...
df_metadata <- df_metadata_prom_rep

### SELECCIÓN DE PICOS #########################################################
################################################################################

# Selección de picos para binary discriminant analysis (BDA)
factor_tipo <- factor(df_metadata$estado)
is.binaryMatrix(matint_179_mue_dico) # TRUE
br <- binda.ranking(matint_179_mue_dico, factor_tipo, verbose = FALSE)

# Gráfico de picos vs score 
nueva_columna <- c()
matriz <- matrix(br, nrow = 496, ncol = 4) #496 es la cantidad de picos
for (i in 1:496) {
  nuevo_valor <- colnames(matint_179_mue_dico)[br[i]]
...
plot(df_br$nueva_columna, df_br$V2, 
     xlab = "m/z", ylab = "Score", 
     main = "Ranking de picos de los espectros (179 muestras)")
...
comparacion <- clValid(
  obj        = matint_179_mue_dico[, top_actual],
  nClust     = 2:6,
...
#top_actual <- br[setdiff(3:15, 6:10)] # PRIMER CONJUNTO 
#top_actual <- br[setdiff(1:15, 6:8)] # SEGUNDO CONJUNTO
top_actual <- br[setdiff(1:15, 4:9)] # TERCER CONJUNTO (Mejor)

K.num <- 2 # clusters
var2 = 0.95

pam.top8 <- pam(matint_179_mue[, top_actual], 
...
cluster_pam8 <- fviz_cluster(pam.top8, 
                              ...
                              data = matint_179_mue[, top_actual],
                              outlier.color = "black",
                              ggtheme = theme_grey(),
                              ...
                              geom = "point",
                              pointsize = 0.5,
                              main = "CTRL/SEPSIS - PAM - 9 picos - 2 clusters") +
...
matint_179_top <- matint_179_mue[, top_actual]

# Realizar PCA para obtener los componentes principales
pca_res <- prcomp(matint_179_top, center = TRUE, scale. = TRUE)
...
highlight_positions <- as.integer(round(as.numeric(colnames(matint_179_top))))
tolerance <- 10
...
selected_spectra <- c(1, 16, 53, 145)
...
for (plot_idx in 1:num_plots) {
  ...
  for (i in start_idx:end_idx) {
    ...
    mass <- Spectra_list_prom_rep[[spec_idx]]@mass
    intensity <- Spectra_list_prom_rep[[spec_idx]]@intensity
    
    # Graficar el espectro individual
    plot(mass, intensity, type = "l", col = "black",
         xlab = "Mass/Charge (m/z)", ylab = "Intensity",
         main = paste("Espectro ", spec_idx,"- grupo: ", df_metadata_prom_rep[spec_idx,7]),
         xlim = x_lim)
...
sample_names <- rownames(matint_179_mue)
...
# Totales por cluster
total_cluster1 <- sum(cluster_assignments == 2)
total_cluster2 <- sum(cluster_assignments == 1)
...
# 1. Silhouette Score
silhouette_score <- silhouette(cluster_assignments, dist(matint_179_mue[, top_actual]))
...
centroid_global <- colMeans(matint_179_mue[, top_actual])
